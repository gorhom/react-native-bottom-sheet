...
Dynamic Sizing is one of the powerful features that `v5` introduces, where the library internally manages to calculate static views and list content height and set it as the bottom sheet content height.

:::info

The mechanism was previously introduced with [useBottomSheetDynamicSnapPoints](https://gorhom.dev/react-native-bottom-sheet/v4/hooks#usebottomsheetdynamicsnappoints) hook which has been deprecated in the latest release.
